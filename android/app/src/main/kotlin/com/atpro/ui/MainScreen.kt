...
// ─────────────────────────────────────────────────────────────
//  MainScreen — shell duy nhất chứa tất cả 4 tab
//
//  [v1.1.5] Thay thế ShortcutRow bằng bottom NavigationBar:
//  Dashboard · Thống kê · Tài khoản · Nhật ký
//
//  Lưu ý inset:
//    - contentWindowInsets = WindowInsets(0) → Scaffold không tự
//      thêm status bar padding vào innerPadding, tránh double-pad
//      với statusBarsPadding() bên trong mỗi Screen.
//    - NavigationBar tự xử lý gesture bar qua navigationBarsPadding().
//    - innerPadding.calculateBottomPadding() = chiều cao NavigationBar
//      (đã bao gồm gesture/button nav bar) → Box padding bottom đúng.
// ─────────────────────────────────────────────────────────────
...
fun MainScreen(
    ...
    Scaffold(
        ...
        contentWindowInsets = WindowInsets(0),   // mỗi Screen tự xử lý statusBarsPadding()
        bottomBar = {
            NavigationBar(
        ...
        // Chỉ padding bottom — tránh double status bar từ Scaffold + Screen
        Box(
            modifier = Modifier
                .fillMaxSize()
