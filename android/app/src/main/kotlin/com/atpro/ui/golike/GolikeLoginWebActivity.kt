...
class GolikeLoginWebActivity : AppCompatActivity() {
    ...
    fun injectNetworkInterceptor() {
        ...
        Log.d(TAG, "🌐 Patch fetch()/XMLHttpRequest — bắt token từ response API")
        ...
        val js = buildString {
            ...
            append("       }")
            append("   }")
            // Patch fetch() — clone() response để không tiêu thụ mất body gốc,
            // trả nguyên response THẬT cho code của Golike (không phá luồng đăng nhập).
            append("   if (window.fetch) {")
            append("       var originalFetch = window.fetch;")
            append("       window.fetch = function() {")
            append("           var fetchArgs = arguments;")
            append("           return originalFetch.apply(this, fetchArgs).then(function(response) {")
            append("               try {")
            ...
            append("       };")
            append("   }")
            // Patch XMLHttpRequest — chỉ THÊM listener 'load', không ghi đè
            // onload/onreadystatechange sẵn có của trang (tránh phá code gốc).
