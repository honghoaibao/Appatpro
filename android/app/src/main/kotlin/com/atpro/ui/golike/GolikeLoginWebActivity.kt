...
""".trimIndent()
    ...
    private fun setupWebView() {
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            /** Không tự động extract token — chỉ inject interceptor thụ động.
             *  Token chỉ được lấy khi người dùng bấm nút "Lấy ATH". */
            override fun onPageFinished(view: WebView?, url: String?) {
                super.onPageFinished(view, url)
                // Chỉ re-inject interceptor phòng trường hợp page reload xoá hook cũ
                view?.evaluateJavascript(JS_INTERCEPTOR, null)
            }
    ...
    inner class Bridge {
        /** Gọi từ JS_EXTRACT khi người dùng bấm "Lấy ATH" */
        @JavascriptInterface
        fun onAthTokenFound(token: String) {
            jsCallbackFired = true
            Log.d(TAG, "JS bridge: len=${token.length} prefix='${token.take(20)}'")
            runOnUiThread {
                if (tokenSaved) return@runOnUiThread
                if (token.isBlank() || token.length < 16) {
                    // JS không tìm thấy trong storage — timeout job sẽ hiện lỗi
