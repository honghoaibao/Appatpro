...
class GolikeLoginWebActivity : AppCompatActivity() {
    ...
    fun q(token: String) {
        ...
        // Lưu và finish trên main thread (runOnUiThread theo smali)
        runOnUiThread {
            updateStatus("✓ Đã lấy được token — đang xác thực...")
            A.isEnabled = false

            // Verify với server trước khi lưu
            scope.launch {
                try {
                    ...
                    when (result) {
                        is GolikeResult.Success -> {
                            ...
                            updateStatus("✓ Đăng nhập thành công — $name")

                            // Lưu qua GolikeRepository (saveWebToken)
                        ...
                        is GolikeResult.Error -> {
                            Log.w(TAG, "Verify ${result.code}: ${result.message}")
                            // Reset để thử lại
                            C = false
                            updateStatus("Token không hợp lệ — thử đăng nhập lại (${result.code})")
                            resetButton()
                        }
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "q verify: ${e.message}")
                    C = false
                    updateStatus("Lỗi kết nối — thử lại")
                    resetButton()
                }
            }
        }
