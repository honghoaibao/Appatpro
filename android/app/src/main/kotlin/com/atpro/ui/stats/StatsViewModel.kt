...
class StatsViewModel(
    ...
    fun refreshGolikeStats() {
        ...
        viewModelScope.launch {
            ...
            val tiktokStats = (statsResult as? GolikeResult.Success)?.data?.tiktok
            val holdCoin    = tiktokStats?.holdCoin    ?: 0.0
            val pendingCoin = tiktokStats?.pendingCoin ?: 0.0
...
/** v1.3.10 — Trạng thái card thống kê Golike (số dư/tiền giữ/rank). */
data class GolikeStatsUiState(
    val isLoading:   Boolean = false,
    val isLoggedIn:  Boolean = false,
    val coin:        Double  = 0.0,
    val holdCoin:    Double  = 0.0,
