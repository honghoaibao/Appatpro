...
class DashboardViewModel(
    ...
    /** v1.2.2: Poll trạng thái đăng nhập Golike mỗi 30s để cập nhật canStart.
     *  v1.2.9: Cũng fetch số dư (từ getMe) và tiền chờ duyệt (từ getStatistics).
     *  v1.3.4: Tách phần thân vòng lặp ra `refreshGolikeStateOnce()` để có thể
     *  gọi lại NGAY LẬP TỨC sau khi đăng nhập qua WebView, không phải đợi tới
     *  30s tiếp theo mới thấy trạng thái cập nhật. */
    private fun pollGolikeLoginState() {
        val repo = golikeRepo ?: return
        viewModelScope.launch {
            while (true) {
                refreshGolikeStateOnceInternal(repo)
                delay(30_000)
            }
        }
    }

    private suspend fun refreshGolikeStateOnceInternal(repo: GolikeRepository) {
        ...
        if (loggedIn) {
            ...
            // v1.3.11 [FIX]: đổi sang `pendingCoin` (chỉ TikTok) thay vì tổng
            // `holdCoin` của cả 3 nền tảng — khớp đúng với cách StatsScreen
            // tính "Chờ duyệt" (mục 15.4/16 revision doc). Trước đây field này
            // (dù tên biến/state vẫn giữ `golikeHoldCoin` để đỡ phải đổi tên
            // rải rác nhiều nơi) cộng dồn cả Facebook/Instagram dù app chỉ hỗ
            // trợ Golike TikTok — ra số khác hẳn với Thống kê, gây hiểu lầm là
            // 2 nơi tính sai lệch nhau khi thực chất đang đo 2 khái niệm khác
            // nhau (holdCoin tổng 3 nền tảng vs pendingCoin riêng TikTok).
            val statsResult = repo.getStatistics()
            if (statsResult is com.atpro.golike.GolikeResult.Success) {
                val pendingCoin = statsResult.data.tiktok?.pendingCoin ?: 0.0
                _uiState.update { it.copy(golikeHoldCoin = pendingCoin) }
            }
        }
    }
    ...
    /**
     * v1.3.4: Nhận kết quả từ `GolikeLoginWebActivity` (đăng nhập qua WebView).
     * Activity đó đã tự lưu token vào GolikeRepository (`saveWebToken()`) trước
     * khi trả kết quả — ở đây chỉ cần refresh UI NGAY, không phải đợi tới lượt
     * poll 30s tiếp theo. `token == null` nghĩa là người dùng bấm back/huỷ giữa
     * chừng — không phải lỗi, chỉ đơn giản là không có gì để cập nhật.
     */
    fun onGolikeWebLoginResult(token: String?) {
        val repo = golikeRepo ?: return
        if (token.isNullOrBlank()) return
        viewModelScope.launch { refreshGolikeStateOnceInternal(repo) }
    }
