...
class DashboardViewModel(
     ...
     *  v1.2.9: Cũng fetch số dư và tiền chờ duyệt khi đăng nhập. */
    private fun pollGolikeLoginState() {
        ...
        viewModelScope.launch {
            while (true) {
                ...
                if (loggedIn) {
                    // Fetch số dư — dùng getStatistics() vì chứa current_coin + hold_coin
                    val stats = repo.getStatistics()
                    if (stats is com.atpro.golike.GolikeResult.Success) {
                        val holdCoin = listOfNotNull(
                            stats.data.tiktok?.holdCoin,
                            stats.data.facebook?.holdCoin,
                            stats.data.instagram?.holdCoin,
                        ).sumOf { it }
                        _uiState.update { it.copy(
                            golikeCoin     = stats.data.currentCoin,
                            golikeHoldCoin = holdCoin,
                        )}
                    }
                }
                delay(30_000)  // refresh mỗi 30s để tránh spam API
            }
        }
    }
