...
object OverlayFarmMonitor {
    ...
    /**
     * [FIX BUG 2] Hiển thị trạng thái giai đoạn khởi động (Phase 1-5) lên overlay.
     * Gọi với msg="" để reset về hiển thị farm bình thường.
     *
     * Dùng overlay thay vì Dialog của AT Pro vì Dialog không hiển thị
     * được khi TikTok đang ở foreground.
     */
    fun setStartupStatus(msg: String) {
        handler.post {
            if (msg.isEmpty()) {
                // Reset về hiển thị farm bình thường
                tvAction?.text = "▶ Chờ..."
            } else {
                tvAction?.text = msg
            }
