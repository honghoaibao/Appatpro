...
object OverlayFarmMonitor {
    ...
    fun update(
        ...
        handler.post {
            ...
            // [v1.2.7] Loading dots animation — action text khi đang chờ/chuẩn bị
            val waitingActions = listOf("Chuẩn bị", "Chờ", "Đang khởi động", "Đang nghỉ", "Đang tải")
            val isWaiting = waitingActions.any { action.startsWith(it) }
            if (isWaiting) {
                // Dùng dots animation cho các trạng thái chờ
                val baseText = waitingActions.firstOrNull { action.startsWith(it) } ?: action
                startDotsAnimation(baseText)
            } else {
                stopDotsAnimation()
    ...
    private fun buildView(context: Context): View {
        ...
        // Action text
        // v1.2.7: Action text — kích thước lớn hơn, padding tốt hơn
        tvAction = text("Chờ", 9.5f, C_MUTED).apply { setPadding(0, dp(4), 0, dp(1)) }
