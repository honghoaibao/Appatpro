...
object TikTokDeepLinks {
     ...
     *
     * Thứ tự ưu tiên (BUG-SETTINGS-001):
     *   1. Deep link `tiktok://settings`      — hoạt động mọi vùng/phiên bản
     *   2. Deep link `snssdk1233://settings`  — fallback một số phiên bản cũ
     *   3. Component name trực tiếp           — last resort; sẽ fail nếu exported=false
     *
     * Lý do KHÔNG gọi setClassName trực tiếp làm bước đầu:
     *   TikTokSettingActivity có android:exported="false" trong AndroidManifest →
     *   startActivity() ném SecurityException → openSettings() trả false →
     *   engine log "❌ Không mở được switch popup".
     */
    fun openSettings(context: Context): Boolean {
        // Attempt 1: deep link chính (mọi vùng/phiên bản hiện đại)
        try {
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse("$SCHEME_TIKTOK://settings")).apply {
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            }
            context.startActivity(intent)
            Log.d(TAG, "openSettings: OK via tiktok://settings")
            return true
        } catch (_: Exception) { /* fall through */ }

        // Attempt 2: scheme snssdk1233 (một số build khu vực)
        try {
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse("snssdk1233://settings")).apply {
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            }
            context.startActivity(intent)
            Log.d(TAG, "openSettings: OK via snssdk1233://settings")
            return true
        } catch (_: Exception) { /* fall through */ }

        // Attempt 3: component name (last resort — có thể fail nếu exported=false)
        return try {
            ...
            context.startActivity(intent)
            Log.d(TAG, "openSettings: OK via component name")
            true
        } catch (e: Exception) {
            Log.e(TAG, "openSettings: all attempts failed — ${e.message}")
            false
        }
    }
