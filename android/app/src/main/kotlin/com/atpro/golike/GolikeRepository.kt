...
class GolikeRepository(private val local: LocalRepository) {
     ...
     *
     * v1.3.3 (bổ sung) — KHÔNG còn được gọi từ `AutomationEngine.taskOneAccount()`.
     * Luồng nhận job đã đổi sang giả lập người thật thao tác trên chính trang
     * `app.golike.net/jobs/tiktok` qua `GolikeJobBridge`/`GolikeJobWebActivity`
     * (xem ADR-0008). Giữ lại hàm này (không xoá) vì vẫn là 1 endpoint hợp lệ,
     * có thể hữu ích cho việc tham khảo/debug hoặc dùng lại trong tương lai.
     */
     ...
     *
     * v1.3.3 (bổ sung) — KHÔNG còn được gọi từ task flow (xem ghi chú ở
     * `getTikTokJobs()` phía trên). Báo cáo "Hoàn thành"/"Lỗi" giờ thực hiện
     * bằng cách bấm nút thật trên trang web (`GolikeJobWebActivity.completeJob()`).
     */
     ...
     *
     * v1.3.3 (bổ sung) — KHÔNG còn được gọi từ task flow. Job không khớp cấu
     * hình giờ được "bỏ qua" bằng cách back ra khỏi trang chi tiết trên web
     * (`GolikeJobWebActivity.abandonJob()`) rồi để job tự hết hạn, thay vì
     * gọi endpoint skip riêng.
     */
