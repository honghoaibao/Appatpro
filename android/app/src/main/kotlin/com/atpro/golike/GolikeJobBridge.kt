...
/**
 * v1.3.3 (bổ sung 6) — Toạ độ MÀN HÌNH THẬT (px, không phải CSS px trong
 * WebView) của 3 điểm captcha kéo-thả Golike ("Kéo khối vuông đi xuyên qua
 * vòng nét đứt rồi thả vào vòng đích") — chỉ xuất hiện sau khi bấm "Nhận Job
 * ngay". Quy đổi CSS→screen thực hiện trong `GolikeJobWebActivity.detectCaptchaPoints()`.
 */
data class CaptchaScreenPoints(
    val square: Pair<Int, Int>,
    val dashedCircle: Pair<Int, Int>,
    val targetCircle: Pair<Int, Int>,
)
...
object GolikeJobBridge {
    ...
    /** Bước 2 — bấm "Nhận Job ngay". */
    suspend fun clickReceiveJobNow(): Boolean =
        withActivity(default = false) { it.clickReceiveJobNow() }

    /**
     * v1.3.3 (bổ sung 6) — Phát hiện captcha kéo-thả (chỉ xuất hiện sau
     * `clickReceiveJobNow()`). Trả null nếu không có captcha lúc này.
     */
    suspend fun detectCaptcha(): CaptchaScreenPoints? =
        withActivity(default = null) { it.detectCaptchaPoints() }

    /** Kiểm tra captcha ĐÃ BIẾN MẤT (đã giải xong) hay chưa — gọi lại sau khi kéo. */
    suspend fun isCaptchaGone(): Boolean =
        withActivity(default = true) { it.isCaptchaGone() }

    /** Bước 2 (tiếp) — sau khi đã xử lý captcha (nếu có): popup xác nhận + đọc trang chi tiết. */
    ...
    /**
     * v1.3.4 — Job loại KHÔNG được hỗ trợ (comment, yêu thích...) — bỏ qua
     * CHỦ ĐỘNG bằng luồng Báo lỗi → cuộn → Gửi báo cáo → OK (khác `abandonJob()`
     * chỉ back() rồi chờ job tự hết hạn).
     */
    suspend fun reportAndSkipJob(): GolikeJobReportResult =
        withActivity(default = GolikeJobReportResult.Unknown) { it.reportAndSkipJob() }
