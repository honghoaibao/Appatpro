...
class TikTokAccessibilityService : AccessibilityService(), IFarmHost {
    ...
    // v1.3.1: dispatch sang Dispatchers.IO — xem giải thích chi tiết ở IFarmHost.getRootNode().
    override suspend fun getRootNode(): AccessibilityNodeInfo? =
        withContext(Dispatchers.IO) { rootInActiveWindow }
    ...
    override suspend fun dragPath(points: List<Pair<Int, Int>>, durationMs: Long): Boolean {
        if (points.size < 2) {
            Log.w(TAG, "dragPath: cần >= 2 điểm, có ${points.size} — skip")
            return false
        }
        if (points.any { it.first < 0 || it.second < 0 }) {
            Log.w(TAG, "dragPath($points): toạ độ âm — skipping gesture")
            return false
        }
        return suspendCancellableCoroutine { cont ->
            try {
                ...
                Log.w(TAG, "dragPath($points): gesture rejected — ${e.message}")
                cont.resume(false) {}
            }
        }
    }
    ...
    /** v1.3.3 (bổ sung) — Context thô cho GolikeJobBridge (mở GolikeJobWebActivity từ coroutine nền). */
