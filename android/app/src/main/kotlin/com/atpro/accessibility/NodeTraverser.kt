...
object NodeTraverser {
    ...
    fun detectAd(root: AccessibilityNodeInfo?): Boolean {
        ...
        // Tier 2: Badge quảng cáo rõ ràng — xuất hiện ĐỘC LẬP, không cần kết hợp.
        // Dùng contains để bắt "Được tài trợ", "Nội dung được tài trợ", v.v.
        val AD_BADGE = listOf(
            "quảng cáo",         // badge góc trái TikTok VN
            "được tài trợ",      // "Được tài trợ" — phổ biến nhất VN
            "nội dung tài trợ",  // variant khác
            "bỏ qua quảng cáo",
            "skip ad",
            "sponsored",         // EN badge
        )
        if (AD_BADGE.any { it in text }) return true

        // Tier 3: CTA mua sắm / install — CHỈ valid khi ĐI KÈM ít nhất 1 dấu hiệu
        // thương mại/tài trợ. Không dùng standalone vì "cài đặt" / "tải về" xuất hiện
        // ở video review/tutorial bình thường (false positive cũ).
        val AD_CTA = listOf(
            "mua ngay", "mua sắm ngay", "đặt mua", "đặt hàng ngay",
            "buy now", "shop now", "order now",
            "tải về", "tải ngay", "download now",
            "cài đặt ngay", "install now", "get now",
        )
        val AD_COMMERCE_SIGNAL = listOf(
            "tìm hiểu thêm", "learn more",
            "đăng ký ngay", "sign up",
            "#hợptácgolike", "#quảngcáo", "#ad", "#sponsored",
            "giá:", "giảm giá", "khuyến mãi", "ưu đãi",
        )
        val hasCta      = AD_CTA.any { it in text }
        val hasCommerce = AD_COMMERCE_SIGNAL.any { it in text }
        // CTA + ≥1 commerce signal → ad
        if (hasCta && hasCommerce) return true
        // Nhiều CTA cùng lúc (≥2) → ad chắc chắn
        if (AD_CTA.count { it in text } >= 2) return true
