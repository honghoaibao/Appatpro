...
object NodeTraverser {
    ...
    fun detectAd(root: AccessibilityNodeInfo?): Boolean {
        ...
        // Tier 2: Badge quảng cáo — contains (không exact) để bắt "Được tài trợ",
        // "Nội dung được tài trợ", "quảng cáo", "sponsored" v.v.
        // BUG FIX v1.2.6: exact=true trước đây bỏ sót "Được tài trợ" vì không
        // khớp chính xác với "tài trợ".
        val AD_BADGE = listOf(
            "quảng cáo",        // badge góc trái
            "được tài trợ",     // "Được tài trợ" — biến thể phổ biến nhất VN
            "tài trợ",          // bắt cả "Được tài trợ" và "Nội dung tài trợ"
            "sponsored",        // EN badge
            "bỏ qua quảng cáo",
            "skip ad",
        )
        ...
        // Tier 3: High-confidence CTA — nút bấm này CHỈ XUẤT HIỆN trong quảng cáo.
        // 1 tín hiệu là đủ kết luận ad (không cần kết hợp).
        // BUG FIX v1.2.6: "Mua ngay >", "Tải về", "Cài đặt ngay" bị bỏ sót vì
        // cần >=2 signals trong Tier cũ — dẫn đến tool tap nhầm CTA → thoát TikTok.
        val AD_CTA_HIGH = listOf(
            "mua ngay",         // "Mua ngay >" — CTA shopping phổ biến nhất
            "mua sắm ngay",
            "đặt mua",
            "tải về",           // App install ad
            "tải ngay",
            "cài đặt ngay",
            "cài đặt",
            "buy now",
        ...
        if (AD_CTA_HIGH.any { it in text }) return true

        // Tier 4: Soft signals — cần >=2 để tránh false positive
        // (các cụm này đôi khi xuất hiện trong video thường)
        val AD_SOFT = listOf(
            "tìm hiểu thêm",
            "đặt hàng ngay",
            "đăng ký ngay",
