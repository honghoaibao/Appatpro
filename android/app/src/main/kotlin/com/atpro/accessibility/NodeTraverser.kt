...
object NodeTraverser {
    ...
    fun parseAccountListWithNodes(
        ...
        // ── Pass 2+3 (GỘP — v1.3.4 FIX): ASCII username + display name Unicode,
        // TRONG CÙNG 1 vòng lặp, theo ĐÚNG thứ tự traversal (= thứ tự hiển thị
        // trên màn hình trong đa số layout danh sách thực tế).
        //
        // BUG-NORMALIZE-002 (v1.3.3 và trước): Pass 2 (username ASCII) và Pass 3
        // (display name Unicode) từng là 2 VÒNG LẶP TÁCH RIÊNG chạy tuần tự —
        // Pass 2 quét & thêm HẾT các username ASCII trước, xong mới tới Pass 3
        // quét & thêm display name Unicode. Hậu quả: bất kỳ entry nào CHỈ khớp
        // Pass 3 (display name Unicode, vd "Hoài Bảo") luôn bị dồn xuống CUỐI
        // `entries` — bất kể vị trí THẬT trên màn hình — vì toàn bộ Pass 2 đã
        // chạy xong và thêm entry trước nó. Nếu acc ĐANG ĐĂNG NHẬP (luôn hiển
        // thị đầu danh sách trong switch popup TikTok) lại là đúng loại "cần
        // chuẩn hoá" (display name Unicode) trong khi CÁC acc khác trong danh
        // sách đã có @username ASCII hợp lệ → acc đang login bị xếp SAI xuống
        // vị trí không phải 0 → toàn bộ cơ chế "không bao giờ click vị trí 0"
        // (`openSwitchPopup()` TH1/TH2, `clickInPopup()`) bị vô hiệu hoá, dẫn
        // tới bug thật đã xảy ra: tool click switch nhầm vào chính acc đang
        // đăng nhập (xem `normalizeInvalidAccountIds()` — log "Click node
        // '<tên>' để switch..." nhắm vào acc đầu danh sách).
        //
        // Fix: 1 vòng lặp DUY NHẤT trên `allNodes` — với MỖI node, thử Pass 2
        // (ASCII) trước, nếu không khớp mới thử Pass 3 (Unicode/display name)
        // — nhờ vậy thứ tự thêm vào `entries` bảo toàn ĐÚNG thứ tự traversal
        // gốc, không phụ thuộc entry đó thuộc "loại" nào. Toàn bộ điều kiện
        // lọc của Pass 2/3 GIỮ NGUYÊN, chỉ gộp vòng lặp.
        val usernameRegex = Regex("^[a-zA-Z0-9_.]{3,24}$")
        allNodes.forEach { nr ->
            val raw = nr.displayText ?: return@forEach

            // Pass 2 — ASCII username (TikTok Trill / biến thể)
            val cleanAscii = raw.trim().replace(" ", "")
            if (usernameRegex.matches(cleanAscii)
                && cleanAscii.lowercase() !in GENERIC_UI_DENYLIST
                && !BADGE_REGEX.matches(cleanAscii)
            ) {
                if (seen.add(cleanAscii.lowercase())) {
                    entries.add(AccountEntry(cleanAscii, nr.node, isNeedsNormalize = !isValidTikTokUsername(cleanAscii)))
                }
                return@forEach   // node này đã phân loại xong — không xét Pass 3 nữa
            }

            // Pass 3 — Display name Unicode (VD: "Bảo Hoài1502", "Ảo vãi") —
            // TikTok đôi khi hiển thị nickname thay @unique_username.
            // Dấu hiệu: chứa ký tự Unicode (ã, ả, ô...) hoặc dấu cách.
            //
            // BUG-NORMALIZE-001 fix: ngoài denylist mở rộng ở trên, áp thêm 2 bộ
            // lọc hình học để chặn các node KHÔNG PHẢI hàng tài khoản trong danh
            // sách:
    ...
    fun detectPopup(root: AccessibilityNodeInfo?): PopupInfo {
        ...
        // [v1.3.1] "Tài khoản của bạn cần được cập nhật" — dialog TikTok yêu cầu
        // liên kết SĐT/email TRƯỚC KHI chuyển acc, đè lên ngay trên danh sách
        // switch-account. Phải check TRƯỚC nhánh ACCOUNT_SWITCH bên dưới: cây
        // node lúc này vẫn còn đủ >=2 "@username" + "chuyển đổi tài khoản" (vì
        // danh sách switch vẫn nằm phía sau dialog), nên nếu không ưu tiên check
        // ở đây trước, nó bị nhận nhầm thành ACCOUNT_SWITCH bình thường (handled
        // = true, không làm gì) → không bấm "Để sau" → tool tưởng đã chuyển acc
        // xong nhưng dialog vẫn còn chặn, dẫn tới verify sai / dừng tool.
        val hasUpdatePrompt =
            findByText(root, "cần được cập nhật", ignoreCase = true) != null
                || findByText(root, "liên kết số điện thoại", ignoreCase = true) != null
                || findByText(root, "account needs to be updated", ignoreCase = true) != null
        if (hasUpdatePrompt) {
            return PopupInfo(true, PopupType.ACCOUNT_UPDATE_PROMPT, null)
        }

        // v1.3.3 (bổ sung 6) — Popup "Liên kết email" (hình 2 người dùng gửi):
        // "Hãy liên kết địa chỉ email Android của bạn để những người mà bạn có
        // thể biết tìm ra bạn và giúp hiển thị quảng cáo phù hợp với bạn hơn."
        // Nút: "Để sau" / "OK" — LUÔN bấm "Để sau" (không tự ý liên kết email
        // thay người dùng). Xuất hiện đè lên FEED (không phải màn hình riêng)
        // — vì vậy phải check KHÔNG phụ thuộc `isOnFeedTab()` (khác nhánh
        // GENERIC ở cuối hàm này, vốn cố tình bỏ qua feed để tránh false-
        // positive) — tiêu đề "Liên kết email" đủ đặc trưng, rủi ro false-
        // positive từ caption/hashtag video gần như không có.
        val hasEmailLinkPrompt = findByText(root, "liên kết email", ignoreCase = true) != null
        if (hasEmailLinkPrompt) {
            return PopupInfo(true, PopupType.EMAIL_LINK_PROMPT, null)
        }
     ...
     *
     * `\[v1.3.2\]` BUG: `PROFILE_IDS`/`PROFILE_TEXTS` chứa các chuỗi ngắn dùng
     * `contains()` (vd "me") — dễ trúng nhầm node KHÔNG nằm trong nav bar, vì
     * substring "me" cũng nằm trong "ho**me**"/"trang chủ (**home**)" hay
     * resource-id "tab_**home**". Khi node đó (Trang chủ/Đề xuất/"Xem thêm"
     * caption...) xuất hiện SỚM HƠN nút Profile thật trong thứ tự duyệt cây,
     * tool bấm nhầm nó thay vì mở Hồ sơ → không đọc được @username.
     *
     * Fix: nếu biết `screenHeight`, giới hạn ứng viên vào dải Y ở khoảng
     * dưới cùng màn hình (nơi nav bar luôn nằm) TRƯỚC khi so khớp id/text —
     * loại các node cùng khớp chuỗi nhưng nằm sai vị trí (giữa/trên màn hình).
     * Nếu không tìm thấy trong dải này (hoặc không truyền screenHeight), rơi
     * về hành vi cũ — quét toàn cây — để không phá các case đang hoạt động
     * đúng trên thiết bị có nav bar cao hơn ước tính.
     */
    /**
     * v1.3.5 — FIX: đổi cơ chế chính từ "tìm theo resourceId/text" sang "tìm
     * THEO VỊ TRÍ trong dải Y đáy màn hình" (nav bar).
     *
     * Lý do đổi: resourceId của TikTok có thể bị obfuscate khác nhau giữa
     * các bản build (R8/ProGuard đổi tên mỗi lần build khác nhau), và text/
     * contentDescription có thể trống (icon-only, không nhãn) hoặc đổi theo
     * ngôn ngữ máy — khiến `matchWithin()` (cách CŨ, theo tên) thỉnh thoảng
     * không tìm thấy dù nút vẫn nằm ĐÚNG vị trí quen thuộc trên màn hình.
     * Vị trí hình học (tab Hồ sơ luôn là tab NGOÀI CÙNG BÊN PHẢI trong 5 tab
     * bottom-nav của TikTok) ổn định hơn nhiều so với tên/id.
     *
     * Cơ chế MỚI (ưu tiên chính): trong dải Y đáy màn hình (nav bar), lấy
     * node CLICKABLE có centerX lớn nhất (ngoài cùng bên phải) — không cần
     * khớp tên/id gì cả. Lọc bỏ node quá nhỏ (badge chấm đỏ...) bằng
     * ngưỡng pixel tuyệt đối nhỏ, không cần biết mật độ màn hình.
     *
     * Cơ chế CŨ (theo tên/id) GIỮ LẠI làm fallback — chỉ chạy khi không xác
     * định được dải Y (thiếu `screenHeight`) hoặc không tìm thấy node
     * clickable nào trong dải đó (layout bất thường ngoài dự kiến).
     */
    fun findProfileTab(root: AccessibilityNodeInfo?, screenHeight: Int = Int.MAX_VALUE): NodeResult? {
        root ?: return null

        // ── Cơ chế MỚI: vị trí (dải Y đáy màn hình + ngoài cùng bên phải) ──
        if (screenHeight != Int.MAX_VALUE) {
            val yMin = (screenHeight * 0.82).toInt()
            val navBarCandidates = traverseAll(root).map { it.toResult() }
                .filter { it.centerY in yMin..screenHeight }
                .filter { it.isClickable }
                // Lọc node quá nhỏ (badge chấm đỏ, viền mảnh...) — icon tab
                // thật luôn to hơn hẳn 24px kể cả trên máy mật độ thấp nhất.
                .filter { it.bounds.width() >= 24 && it.bounds.height() >= 24 }
            navBarCandidates.maxByOrNull { it.centerX }?.let { return it }
        }

        // ── Cơ chế CŨ (fallback): theo resourceId/text/contentDescription ──
        ...
        fun matchWithin(candidates: List<NodeResult>): NodeResult? {
            PROFILE_IDS.forEach { id ->
                candidates.firstOrNull {
                    it.resourceId?.lowercase()?.contains(id) == true
                }?.let { return it }
            }
            PROFILE_TEXTS.forEach { text ->
                candidates.firstOrNull {
                    it.displayText?.lowercase()?.contains(text) == true
                }?.let { return it }
            }
            return candidates.firstOrNull { nr ->
                val cd = nr.node.contentDescription?.toString()?.lowercase() ?: return@firstOrNull false
                PROFILE_CD_KEYWORDS.any { cd.contains(it) }
            }
        }

        // 1. Ưu tiên quét trong dải Y ở đáy màn hình (vùng nav bar).
        if (screenHeight != Int.MAX_VALUE) {
            val yMin = (screenHeight * 0.82).toInt()
            val navBarCandidates = traverseAll(root).map { it.toResult() }
                .filter { it.bounds.centerY() in yMin..screenHeight }
            matchWithin(navBarCandidates)?.let { return it }
        }

        // 2. Fallback: quét toàn cây (hành vi cũ — giữ để không regress khi
        //    không có screenHeight hoặc nav bar nằm ngoài dải ước tính).
        return matchWithin(traverseAll(root).map { it.toResult() })
    }
    ...
    enum class PopupType { NONE, ACCOUNT_SWITCH, ACCOUNT_UPDATE_PROMPT, EMAIL_LINK_PROMPT, VERIFY_1234, DAILY_LIMIT, FOLLOW_FRIENDS, FOLLOW_SUGGEST, GENERIC, SAVE_LOGIN, PERMISSION_REQUEST }
