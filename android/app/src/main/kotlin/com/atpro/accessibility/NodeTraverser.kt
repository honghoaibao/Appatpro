...
object NodeTraverser {
    ...
    /**
     * Detect bài đăng dạng hình ảnh (photo post / image carousel) — v1.1.9.
     *
     * TikTok photo posts khác video ở chỗ:
     *   - Không có thanh tiến trình (progress bar) của video.
     *   - Có nút chuyển ảnh trái/phải hoặc indicator dots.
     *   - Resource-ID đặc trưng: "photo_item", "image_viewer", "swipe_dot", "photo_carousel".
     *   - ContentDescription chứa "ảnh", "photo", "image" kèm chỉ số trang như "1/5".
     *
     * Dùng để phát hiện image post → force swipe thay vì swipe thường
     * (tránh gesture bị TikTok intercept cho horizontal carousel scroll).
     *
     * Tier 1: Resource-ID fragment — nhanh, chính xác nhất.
     * Tier 2: ContentDescription pattern — "ảnh 1 trên 3", "photo 1 of 5".
     * Tier 3: Combination soft signals — cần >= 2 để tránh false positive.
     */
    fun detectImagePost(root: AccessibilityNodeInfo?): Boolean {
        root ?: return false

        // Tier 1: Resource-ID fragments đặc trưng image/carousel post
        val IMAGE_ID_FRAGMENTS = listOf(
            "photo_item", "image_item", "photo_carousel", "image_carousel",
            "swipe_dot", "carousel_dot", "dot_indicator", "page_indicator",
            "image_viewer", "photo_viewer", "carousel_indicator",
            "photo_slide", "image_slide", "album_item",
            "photo_count",                       // "1/5" count badge
        )
        val nodes = traverseAll(root)
        if (IMAGE_ID_FRAGMENTS.any { frag ->
                nodes.any { n -> n.viewIdResourceName?.lowercase()?.contains(frag) == true }
            }) return true

        // Tier 2: ContentDescription chứa pattern "X/Y" kèm keyword ảnh
        val IMAGE_CD_KEYWORDS = listOf("ảnh", "photo", "image", "hình")
        val hasPagedCd = nodes.any { n ->
            val cd = n.contentDescription?.toString()?.lowercase() ?: return@any false
            IMAGE_CD_KEYWORDS.any { kw -> kw in cd } &&
                Regex("""\d+\s*/\s*\d+""").containsMatchIn(cd)
        }
        if (hasPagedCd) return true
        ...
        val SOFT_SIGNALS = listOf(
            ...
            "ảnh trong bài viết này",
            "photos in this post",
        )
        return SOFT_SIGNALS.any { it in text }
    }
