...
class PopupHandler(private val service: IFarmHost) {
    ...
    private fun scanKeywords(root: AccessibilityNodeInfo): KeywordMatch? {
        ...
        val patterns = listOf(
            ...
            // [FIX] "cập nhật" quá rộng — xuất hiện ở mọi màn hình TikTok
            // (menu profile, settings...) → false-positive → pressBack trên feed.
            // Dùng chuỗi đủ dài để chỉ khớp đúng popup update.
            "update available"        to "later",
            "update to continue"      to "later",
            "update tiktok"           to "later",
            "cập nhật ứng dụng"       to "later",
            "cập nhật tiktok"         to "later",
