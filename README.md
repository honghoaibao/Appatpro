...
./scripts/build_release.sh --version 1.0.3 --build 3
```
...
gh workflow run "AT PRO — Build APK" -f version=1.0.3 -f build=3
...
### v1.0.3 (#Patch1) — UI/UX Polish

- **Material Icons thay thế emoji**: toàn bộ emoji icon trên Dashboard, Logs, Stats, Accounts được thay bằng `Icons.Rounded.*` của Jetpack Compose — nhất quán, scale đúng, không phụ thuộc font hệ thống.
- **Animations mới**:
  - `DashboardScreen`: slide + fade khi chuyển IdleView ↔ FarmingView; animated count khi số tài khoản thay đổi; `AnimatedVisibility` cho AccountListInput (expand/shrink mượt); press scale cho nút bắt đầu farm và shortcut tiles; `animateFloatAsState` cho progress bar farm.
  - `FarmingView`: slide account name khi đổi tài khoản; animated stats grid entrance; animated icon/label Pause ↔ Resume.
  - `LogsScreen`: `AnimatedVisibility` cho badge đếm log; `AnimatedContent` khi chuyển giữa empty state và danh sách.
  - `ModeTab`: `animateColorAsState` cho background/text khi chọn mode.
- **PermissionScreen (mới)**: màn hình cấp quyền hiện khi Accessibility Service chưa bật, thay thế hint text thuần. Hiển thị 3 quyền (Accessibility bắt buộc, Overlay + Notifications tùy chọn) với trạng thái cấp quyền live (poll 500ms), animation ✓ khi cấp xong, nút "Cấp quyền" mở thẳng màn hình Settings tương ứng.
- **DashboardViewModel**: thêm `accessibilityGranted`, `overlayGranted`, `notificationGranted` vào `DashboardUiState`; thêm computed `permissionsReady`; poll permission status mỗi 500ms cùng engine state.

### v1.0.3 — BUG-SETTINGS-002 fix (setPackage)
- **Root cause (thực sự)**: deep link `tiktok://settings` trong v1.0.2 thiếu `setPackage()` →
  Android xử lý như Implicit Intent → có thể route sang browser, hiện chooser dialog,
  hoặc throw `ActivityNotFoundException` thầm lặng → Settings vẫn không mở.
- **Fix**: thêm `setPackage(pkg)` vào tất cả deep link attempts. Android buộc phải route
  tới `DeepLinkHandlerActivity` (exported=true) bên trong TikTok, Activity đó tự mở
  `TikTokSettingActivity` (exported=false) từ nội bộ — hoàn toàn hợp lệ.
- **Thêm**: scheme `musically://settings` làm attempt #3 (alias alias TikTok cũ).
- **File đổi**: `TikTokDeepLinks.kt`, `build.gradle` (versionCode 3, v1.0.3),
  `build.yml` (default 1.0.3), `README.md`.

### v1.0.2 — BUG-SETTINGS-001 fix (deep link thay component name)
