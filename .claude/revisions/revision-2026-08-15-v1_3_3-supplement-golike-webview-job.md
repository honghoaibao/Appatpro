...
## Bổ sung 5 (2026-08-17) — FIX crash thật: "WebView method called on thread DefaultDispatcher-worker-N"

Bảo gửi log crash thật từ thiết bị (RuntimeException, ERR log trong app):
```
java.lang.Throwable: A WebView method was called on thread
'DefaultDispatcher-worker-4'. All WebView methods must be called on the
same thread. (Expected Looper Looper (main, tid 2) ... called on null, ...)
```

**Nguyên nhân xác định (đối chiếu code, không suy luận)**: `injectScript()`
gọi thẳng `webView.evaluateJavascript()` KHÔNG qua `withContext(Dispatchers.Main...)`
như `evalJs()` đã làm — trong khi nó được gọi làm bước ĐẦU TIÊN trong cả
`selectAccount()` và `receiveJob()` (dòng đầu tiên mỗi hàm), và 2 hàm này
chạy trên coroutine scope của `AutomationEngine`/`TikTokAccessibilityService`
(`Dispatchers.Default` — khớp chính xác `DefaultDispatcher-worker-4` trong
log), không phải main thread. Đây là lỗ hổng SÓT LẠI từ bổ sung 4 (lúc đó
chỉ fix `evalJs()`, không rà lại `injectScript()` — hàm gọi WebView duy nhất
KHÔNG đi qua `evalJs()`).

### Fix
`GolikeJobWebActivity.injectScript()`: bọc lời gọi `webView.evaluateJavascript()`
trong `runOnUiThread {}` — an toàn gọi từ BẤT KỲ thread nào (Android tự thực
thi ngay nếu đã ở main, tự post vào main Looper nếu không). Đã rà lại TOÀN
BỘ các lời gọi `webView.*` khác trong file — tất cả các lời gọi còn lại đều
nằm trong lifecycle callback (`onCreate`/`onBackPressed`/`onDestroy`) hoặc
`WebViewClient`/`WebChromeClient` callback (luôn do chính WebView framework
gọi trên main thread) hoặc đã đi qua `evalJs()` (đã có `withContext` từ bổ
sung 4) — không còn lời gọi WebView nào thiếu bảo vệ thread.

### Rủi ro đã biết
- Chưa xác nhận lại trên thiết bị thật sau fix (chỉ verify được cú pháp/biên
  dịch tĩnh trong sandbox) — cần Bảo build + chạy thử lại để xác nhận hết
  crash.

## Bổ sung 6 (2026-08-19) — Log rõ khi làm nhiệm vụ, chờ 1-20s sau nhận job, giải captcha kéo-thả, popup "Liên kết email"

Yêu cầu: *"Thêm log cho phần làm nhiệm vụ để người dùng cuối biết tool đang
làm gì / Sau khi nhận jobs có thể phải đợi từ 1-20s / Thêm cơ chế giải xác
minh nhanh, giải xong cần kiểm tra xem đã giải được chưa, phần này chỉ xuất
hiện sau khi nhấn nhận job / Thêm phần giải popup tiktok trong hình 2, bấm
để sau"*. Kèm 2 ảnh: (1) captcha kéo-thả Golike "Kéo khối vuông đi xuyên qua
vòng nét đứt rồi thả vào vòng đích"; (2) popup TikTok "Liên kết email" (nút
"Để sau"/"OK") đè lên feed.

### 6.1 — Log rõ hơn cho bước "làm nhiệm vụ"
`taskOneAccount()` [D]: thêm `setStatus()` riêng theo loại job ("Đang Like
bài viết..."/"Đang Follow tài khoản...") ngay trước khi gọi
`doLikeTask()`/`doFollowTask()` — trước đây bước này (lúc TikTok thật sự
hiển thị trên màn hình, dễ quan sát nhất) không có trạng thái riêng.

### 6.2 — Chờ 1-20s sau khi nhận job
`GolikeJobWebActivity`: timeout chờ trang chi tiết sẵn sàng
(`POST_RECEIVE_WAIT_MS`) tăng từ 7s → 22s (khớp thực tế Bảo xác nhận "có thể
phải đợi từ 1-20s", cộng biên an toàn). Cơ chế polling (`pollUntil`, mỗi
500ms) tự thoát sớm ngay khi sẵn sàng nên KHÔNG tốn thời gian thừa khi
nhanh — chỉ nới TRẦN chờ tối đa. `taskOneAccount()` thêm `setStatus("...có
thể mất đến 20s...")` để người dùng biết đây là chờ có chủ đích, không phải
tool bị treo.

### 6.3 — Giải captcha kéo-thả (chỉ xuất hiện sau khi bấm "Nhận Job ngay")
**Kiến trúc**: tách `receiveJob()` (hàm gộp cũ) thành các bước nhỏ để chèn
bước giải captcha vào ĐÚNG VỊ TRÍ (giữa "bấm Nhận Job ngay" và "popup xác
nhận + đọc chi tiết"):
- `GolikeJobWebActivity.clickReceiveJobNow()` — Bước 2a (tách từ `receiveJob()` cũ).
- `GolikeJobWebActivity.detectCaptchaPoints()` — phát hiện captcha (JS,
  khớp text "xuyên qua vòng nét đứt") + định vị 3 điểm (khối vuông, vòng nét
  đứt, vòng đích) bằng heuristic (không có DOM thật để tham chiếu): vòng =
  phần tử gần vuông có border-radius lớn, `border-style: dashed` → vòng nét
  đứt, còn lại → vòng đích; khối vuông = phần tử KHÔNG tròn, có nền màu, nhỏ
  (24-90px), GẦN vòng nét đứt nhất. Quy đổi toạ độ CSS (JS) → MÀN HÌNH THẬT
  (`webView.getLocationOnScreen() + cssPx * webView.scale * density`).
- `GolikeJobWebActivity.isCaptchaGone()` — verify (đọc lại `detectDragCaptcha()`).
- `GolikeJobWebActivity.handlePopupAndDetail()` — Bước 2b (phần còn lại của
  `receiveJob()` cũ: popup xác nhận + đọc chi tiết).
- `IFarmHost.dragPath(points, durationMs)` (MỚI) — cử chỉ kéo qua NHIỀU điểm
  trong 1 lần nhấc/thả DUY NHẤT (khác `swipeSuspend()` chỉ 2 điểm thẳng).
  Implement trong `TikTokAccessibilityService` bằng `Path` nhiều `lineTo()`
  + `GestureDescription.StrokeDescription`, mirror `swipeSuspend()`.
- `AutomationEngine.solveDragCaptchaIfPresent()` (MỚI) — điều phối: phát
  hiện → `host.dragPath()` (CỬ CHỈ THẬT, không JS-dispatch) → **verify lại**
  (đúng yêu cầu "giải xong cần kiểm tra xem đã giải được chưa") → nếu còn,
  lấy lại toạ độ mới + thử lần 2 → bỏ cuộc job này nếu vẫn còn sau 2 lần.

**Tại sao dùng cử chỉ thật (`host.dragPath()`) thay vì JS-dispatch như các
nút bấm khác trong luồng Golike?** Captcha kéo-thả thường được thiết kế
CHÍNH XÁC để phát hiện/chặn synthetic pointer event — `new MouseEvent(...)`
có `isTrusted = false`; nhiều thư viện anti-bot kiểm tra cờ này hoặc phân
tích pattern di chuyển (tốc độ, gia tốc, độ cong đường đi) mà synthetic
event không có. Cử chỉ thật qua AccessibilityService nhất quán với ADR-0003
(driver chính cho mọi tương tác cần "giống người thật") và đáng tin cậy hơn
hẳn ở trường hợp CỤ THỂ NÀY.

### 6.4 — Popup TikTok "Liên kết email" (bấm "Để sau")
Phát hiện: popup này (ảnh 2) xuất hiện ĐÈ LÊN FEED (không phải màn hình
riêng) — cả `NodeTraverser` (nhánh GENERIC) lẫn `PopupHandler` (Tier 2
`scanKeywords()`) đều CỐ Ý bỏ qua khi `isOnFeedTab(root) == true` (tránh
false-positive từ nội dung feed) → popup này trước đây HOÀN TOÀN KHÔNG được
xử lý.

Fix: thêm `PopupType.EMAIL_LINK_PROMPT` — check SỚM trong
`detectPopup()` (như `ACCOUNT_UPDATE_PROMPT`, không phụ thuộc
`isOnFeedTab()`), khớp tiêu đề đặc trưng "liên kết email" (rủi ro false-
positive từ caption/hashtag gần như không có). `PopupHandler` xử lý: LUÔN
bấm "Để sau" — TUYỆT ĐỐI không bấm "OK" (sẽ liên kết email thay người dùng);
nếu không tìm thấy nút "Để sau" → back thay vì đoán bừa nút khác.

### Rủi ro đã biết (bổ sung 6)
- Heuristic định vị captcha (6.3) hoàn toàn dựa trên suy luận từ ảnh chụp,
  KHÔNG có quyền truy cập DOM thật của `app.golike.net` để xác minh (ngoài
  phạm vi domain công cụ cho phép) — rủi ro cao nhất trong toàn bộ tính
  năng này. Nếu heuristic định vị sai phần tử, `dragPath()` sẽ kéo sai chỗ
  và luôn fail — log `POPUP-GOLIKE: Captcha kéo-thả phát hiện — square=...
  dashed=... target=...` sẽ cho biết toạ độ tính được để đối chiếu/debug.
- Quy đổi CSS px → screen px giả định trang có `<meta name="viewport">`
  chuẩn (1 CSS px ≈ 1dp) — chưa xác minh được.
- `dragPath()` dùng đường thẳng nhiều đoạn (square → dashed → target → target,
  lặp lại điểm cuối để "giữ" 1 nhịp) — chưa có độ cong/tốc độ biến thiên
  giống người thật hơn (như `Human.*` dùng cho click) — có thể cân nhắc bổ
  sung sau nếu captcha vẫn từ chối dù toạ độ đúng.

## Bổ sung 7 (2026-08-19, tiếp) — Rung ngẫu nhiên khi kéo captcha + delay nhiều hơn ở luồng làm nhiệm vụ

Yêu cầu: *"Phần xác minh nhanh thì các phần tử đều ngẫu nhiên, không có toạ
độ chính xác. Cần độ rung nhỏ ngẫu nhiên khi kéo / Phần làm nhiệm vụ: các
thao tác như click, nhận job,... đang quá nhanh, cần delay lại"* (+ 1 yêu
cầu về "chuẩn hoá" — xem mục riêng bên dưới, cần làm rõ thêm với Bảo trước
khi code).

### 7.1 — Độ rung ngẫu nhiên khi kéo captcha
`AutomationEngine.jitteredSegment(a, b, steps=6, jitterPx=4)` (mới) — sinh
các điểm trung gian giữa 2 điểm, mỗi điểm lệch ngẫu nhiên
`Human.jitter(4)` (±4px) theo cả 2 trục quanh vị trí lý tưởng trên đường
thẳng. `solveDragCaptchaIfPresent()`: đường kéo đổi từ 4 điểm thẳng tuyệt
đối (khối vuông → vòng nét đứt → vòng đích → vòng đích) sang khối vuông →
`\[6 điểm rung\]` → vòng nét đứt → `\[6 điểm rung\]` → vòng đích → vòng đích
(điểm đầu/cuối GIỮ NGUYÊN chính xác — không rung, cần thả ĐÚNG vào vòng
đích). `durationMs` tăng nhẹ 900ms → 1100ms cho phù hợp đường đi dài hơn.

### 7.2 — Delay nhiều hơn ở luồng làm nhiệm vụ
Rà lại toàn bộ chuỗi click liên tiếp trong `taskOneAccount()` [B]-[F] và
`GolikeJobWebActivity` — nhiều bước bấm NGAY LẬP TỨC sau khi vừa đọc được
trạng thái mới (0 delay), không giống người thật cần thời gian "nhìn/hiểu"
trước khi hành động. Thêm/tăng delay tại:
- Sau `clickReceiveJobNow()` thành công, trước khi check captcha:
  `Human.delay(500, 1_100)` (mới, trước đây 0ms).
- Trước `clickTikTokButton()`: `Human.delay(500, 1_300)` (mới, trước đây 0ms).
- `[E]` quay lại Golike: 700ms/800ms cố định → `Human.delay(600,1_100)`/`Human.delay(700,1_300)`.
- Trước bấm "Hoàn thành" `[F]`: `Human.delay(400, 900)` (mới).
- `GolikeJobWebActivity.selectAccount()`: 500ms cố định → `humanDelay(500,1_000)`.
- `GolikeJobWebActivity.handlePopupAndDetail()`: thêm delay TRƯỚC "Đã hiểu"
  (mới, `humanDelay(600,1_400)`) + delay giữa "Đã hiểu"→"Đồng ý" đổi từ
  400ms cố định → `humanDelay(500,1_100)`.
- `GolikeJobWebActivity.completeJob()`: toàn bộ chuỗi OK→Báo lỗi→Gửi báo
  cáo→OK đổi từ delay cố định (500/700/900ms) sang `humanDelay()` ngẫu
  nhiên, thêm delay trước OK đầu tiên (mới).

Hàm mới `GolikeJobWebActivity.humanDelay(minMs, maxMs)` — bản mini của
`AutomationEngine.Human.delay()` (không dùng chung được vì `Human` là
`private object` trong `AutomationEngine.kt`, khác file) — dùng
`kotlin.random.Random` trực tiếp.

### 7.3 — Chuẩn hoá acc đầu (CẦN LÀM RÕ THÊM — chưa code)
Bảo mô tả: khi acc cần chuẩn hoá là acc đầu, tool click để chuyển là SAI —
đề xuất: chỉ có 1 acc cần chuẩn hoá và là acc đầu → back về feed → vào Hồ sơ
đọc id (không switch).

Rà lại `AutomationEngine.kt` phát hiện: cơ chế NÀY DƯỜNG NHƯ ĐÃ TỒN TẠI —
`openSwitchPopup()` (v1.3.0, dòng ~981) đã phân 3 trường hợp TH1/TH2/TH3, và
TH2 ("chỉ acc đầu cần chuẩn hoá, không có acc nào khác cần chuẩn hoá") gọi
`confirmCurrentAccountViaProfile()` — hàm này làm ĐÚNG những gì Bảo mô tả:
...
@username thật"*. Cơ chế tương tự cũng có ở `positionFirstAccount()` và
`clickInPopup()` (đều tránh click vào entry vị trí 0).

Vì KHÔNG tìm được lỗi cụ thể trong code hiện có qua đọc tĩnh (không có thiết
bị thật để chạy thử), và việc viết lại/nhân đôi logic này có rủi ro xung đột
với cơ chế đang hoạt động đúng — đã HỎI LẠI Bảo cụ thể xảy ra ở màn hình/luồng
nào (thay vì đoán và có thể sửa nhầm chỗ, hoặc nhân đôi code không cần
thiết) trước khi code phần này. Xem phản hồi trong chat.
