...
- Step 3: If the AGP version is less than 9, suggest moving to AGP 9.0 version as AGP 9.0 includes optimizations (references/android/topic/performance/app-optimization/enable-app-optimization.md).
  - Step 4: Look at the proguard files in the codebase and evaluate each keep rule in the following specific order:
    a. **Libraries check**: Check rules against references/REDUNDANT-RULES.md. If the app has keep rules targeting libraries - Google, AndroidX, Kotlin, Kotlinx, Room, Gson, Retrofit, inform the user that these are not required and suggest removal of these rules.
    b. **Impact analysis**: For the remaining keep rules, assess them based on the impact hierarchy defined in references/KEEP-RULES-IMPACT-HIERARCHY.md. (Note: Do NOT assess the impact of keep rules already covered in the libraries check step).
...
- Step 7: For every keep rule inform concisely and to the point what action needs to be taken - whether the rule needs to be removed/refined.
  - If refining the rule, give instructions on finding a narrower and specific keep rule using references/REFLECTION-GUIDE.md.
